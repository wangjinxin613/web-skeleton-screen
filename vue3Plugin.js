...
const { genArgs } = require('./src/puppeteer/utils.js');
const evalDom = require('./src/evalDOM.js');
...
class SkeletonVue {
  ...
  constructor(app, options) {
    ...
    this.listenBlackList = listenBlackList;
  ...
  isBlack() {
    if (Array.isArray(this.blackList)) {
      return this.blackList.indexOf(this.route) !== -1
    }
    return false;
  }
  ...
  // 当前路由是否命中监听黑名单
  isListenBlack() {
    if (Array.isArray(this.listenBlackList)) {
      return this.listenBlackList.indexOf(this.route) !== -1
    }
    return true;
  }
  ...
  async devInit() {
    ...
    if (await this.isListen()) {
      setTimeout(() => {
        ...
        this.log('此页面已经开启骨架屏监听生成', '生成频率为' + this.pollTime + '毫秒');
      }, 0)
    }
    this.uploadRoutes();
  }
  ...
  createSkeletonPoll() {
    ...
    }, (!isNaN(this.pollTime) && this.pollTime !== null) ? this.pollTime : defaultPollTime)
    this.timer = timer;
  }
  ...
  async createSkeleton(isListen = false) {
    ...
    try {
      const html = await evalDom.apply(window, genArgs.create({
        ...
        blockMerge: {
          type: 'object',
          value: JSON.stringify((typeof this.blockMerge === 'object' && this.blockMerge !== null) ? this.blockMerge : {})
        }
      }));
      if (!html) {
