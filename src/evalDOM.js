    ...
    DrawPageframe.prototype = {
        ...
        getTurelyStyle(node) {
            ...
            if (hasBgColor(node) || backgroundHasurl) {
                ...
                if (childNodes.length === 1 && childNodes[0].nodeType === 3 && childNodes[0].textContent.trim().length) {
        ...
        overflowHiddenStyle(maxPosition, arg) {
            if (maxPosition && maxPosition.width && maxPosition.height) {
                arg.width = ((arg.realWidth + arg.left) > (maxPosition.realWidth + maxPosition.left)) ? (maxPosition.realWidth + maxPosition.left - arg.left) : arg.width;
                arg.height = ((arg.realHeight + arg.top) > (maxPosition.realHeight + maxPosition.top)) ? (maxPosition.realHeight + maxPosition.top - arg.top) : arg.height;
